...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    ...
    for pipe in upperPipes:
        ...
        playerHeight = GAME_SPRITES['player'].get_height()
        if ( playery < pipeHeight+pipe['y'] and pipe['x']<=playerx<=pipe['x']+GAME_SPRITES['pipe'][0].get_width()-50 ):
            GAME_SOUNDS['die'].play()
            return True
    ...
    for pipe in lowerPipes:
        pipeHeight = GAME_SPRITES['pipe'][0].get_height()
        playerHeight = GAME_SPRITES['player'].get_height()
        if ( playery+playerHeight > pipe['y'] and pipe['x']<=playerx<=pipe['x']+GAME_SPRITES['pipe'][0].get_width()-50 ):
            GAME_SOUNDS['die'].play()
            return True
    return False
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['numbers'] = (
        pygame.image.load('pics/0.png').convert_alpha(),
